    ...
    <>
      <div className="space-y-6">
          ...
          <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
            <div>
              <p className="text-xs uppercase tracking-[0.35em] text-slate-500">
                {formMode === 'create' ? 'Nový balíček' : 'Úprava balíčku'}
              </p>
              <h2 className="text-2xl font-semibold mt-2">
                {formMode === 'create' ? 'Přidej balíček' : 'Upravuješ balíček'}
              </h2>
            </div>
            {formMode === 'edit' && (
          ...
          <form className="space-y-6" onSubmit={handleSubmit}>
            <div className="grid gap-4 md:grid-cols-2">
              ...
              <div>
                ...
                <input className="input-field bg-white/5 text-slate-400" value={formState.slug} readOnly />
              </div>
              <div>
              ...
              <div>
                <label className="text-sm text-slate-400 block mb-1">Limit návštěv (tréninky)</label>
                <input
                  type="number"
                  className="input-field"
            ...
            <div className="flex flex-col sm:flex-row gap-3">
              ...
              {formMode === 'edit' && (
                <button type="button" className="secondary-button w-full sm:w-auto" onClick={resetForm}>
                  Začít nový balíček
                </button>
              )}
            </div>
        ...
        <section className="glass-panel rounded-3xl p-6 space-y-4">
          ...
          <div className="grid gap-4 md:grid-cols-2">
            {packages.map((pkg) => (
              <div key={pkg.id} className="glass-subcard rounded-2xl p-4 space-y-3">
                ...
                {pkg.description && <p className="text-slate-300 text-sm">{pkg.description}</p>}
                <div className="flex flex-wrap gap-3">
                  <button className="secondary-button flex-1 sm:flex-none" onClick={() => editPackage(pkg)}>
                    Upravit
                  </button>
                  <button
                    className={`px-4 py-2 rounded-full border ${
                      pkg.is_active
                        ? 'border-rose-400/40 text-rose-200 hover:border-rose-300 hover:text-rose-50'
                        : 'border-emerald-400/40 text-emerald-200 hover:border-emerald-200 hover:text-emerald-50'
                    }`}
          ...
          {!packages.length && !packagesQuery.isFetching && (
            <p className="text-slate-500 text-sm">Zatím žádné balíčky. Přidej první pomocí formuláře výše.</p>
          )}
        </section>
      </div>
      <Toast toast={toast} />
