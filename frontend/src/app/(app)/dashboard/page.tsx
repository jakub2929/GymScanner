    ...
    <>
      <div className="space-y-10">
        <section className="glass-panel rounded-3xl p-6 sm:p-10">
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div>
              <h1 className="text-3xl sm:text-[2.5rem] font-semibold tracking-tight">Tvůj přístupový QR kód</h1>
              {summary && <p className="text-slate-400 mt-4 text-sm">{summary}</p>}
            </div>
          </div>
          ...
          {isPending ? (
            ...
            <div className="mt-10 flex flex-col items-center gap-6" id="qrContainer">
              <div className="inline-flex flex-col items-center gap-4 bg-white/5 rounded-3xl p-6">
                {data?.qr_code_url ? (
                  <Image
              ...
              <div className="w-full glass-subcard rounded-2xl p-5 text-left">
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                  ...
                  <div className="text-right">
                    ...
                    <p className="text-3xl font-semibold text-emerald-300">{data?.credits ?? 0}</p>
                  </div>
                </div>
              </div>
            </div>
        ...
        <div className="grid lg:grid-cols-2 gap-6">
          <section className="glass-panel rounded-3xl p-6 space-y-4">
            <div className="flex items-center justify-between">
              <h2 className="text-2xl font-semibold">Koupit vstupy</h2>
              <span className="text-sm text-slate-400">Dostupné: {data?.credits ?? 0}</span>
            </div>
          ...
          <section className="glass-panel rounded-3xl p-6 space-y-3 text-sm text-slate-300">
      ...
      {buyModal && (
        <div className="fixed inset-0 bg-black/70 z-50 flex items-center justify-center p-4">
          <div className="bg-[#050912] text-white rounded-3xl border border-white/10 p-6 w-full max-w-2xl space-y-4">
            <div className="flex items-center justify-between">
              <h3 className="text-2xl font-semibold">Vyber balíček</h3>
              <button onClick={() => setBuyModal(false)} className="text-3xl text-slate-500 hover:text-white">
                &times;
              </button>
            </div>
            {packages.map((pkg) => (
              <div key={pkg.credits} className="glass-subcard rounded-2xl p-5 flex items-center justify-between">
                <div>
                  ...
                  <p className="text-slate-400">{pkg.amount} Kč</p>
                </div>
